...
async function adminMovies(url:URL,request:Request,env:Env):Promise<Response>{
  ...
  return htmlResponse(layout("Manage movies",`${pageHead("Movies calendar","Pick the releases you care about","Adding a movie puts its Czech release date into the Movies calendar feed.")}${form}${manageMovieGrid(rows)}`,request,env));
}
