...
export async function handleRequest(request:Request,env:Env,ctx:ExecutionContextLike):Promise<Response>{
  ...
  const follow=path.match(/^\/series\/(\d+)\/(follow|unfollow)$/);if(follow&&request.method==="POST"){if(!isAdmin(request,env))return forbidden();const id=Number(follow[1]);if(follow[2]==="follow")await env.DB.prepare("INSERT OR IGNORE INTO followed_shows(show_id,created_at) VALUES (?,?)").bind(id,nowIso()).run();else await env.DB.prepare("DELETE FROM followed_shows WHERE show_id=?").bind(id).run();return redirect(`/series/${id}`);}
...
async function seriesBrowse(url:URL,request:Request,env:Env):Promise<Response>{
  ...
  const orders:Record<string,string>={rating:"COALESCE(s.rating,0) DESC,COALESCE(s.weight,0) DESC,s.name",weight:"COALESCE(s.weight,0) DESC,COALESCE(s.rating,0) DESC",newest:"s.premiered DESC,s.name",name:"s.name COLLATE NOCASE"};
  ...
  return htmlResponse(layout("Series",`${pageHead("TV catalogue","Series","TVmaze catalogue cached in D1 and refreshed incrementally.")}${form}${showGrid(rows)}${pager(url,page,rows.length===PAGE_SIZE)}`,request,env));
}
