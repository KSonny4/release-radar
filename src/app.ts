...
async function seriesBrowse(url:URL,request:Request,env:Env):Promise<Response>{
  ...
  const where:string[]=[],p:unknown[]=[];if(q){where.push("s.name ILIKE ?");p.push(`%${q}%`);}if(genre){where.push("s.genres ILIKE ?");p.push(`%\"${genre}\"%`);}if(status){where.push("s.status=?");p.push(status);}if(min!==null){where.push("COALESCE(s.rating,0)>=?");p.push(min);}
...
async function movieBrowse(url:URL,request:Request,env:Env):Promise<Response>{
  ...
  const where:string[]=[],p:unknown[]=[];if(q){where.push("m.title ILIKE ?");p.push(`%${q}%`);}if(genre){where.push("m.genres ILIKE ?");p.push(`%\"${genre}\"%`);}if(min!==null){where.push("COALESCE(m.rating,0)>=?");p.push(min);}if(votes!==null){where.push("COALESCE(m.vote_count,0)>=?");p.push(votes);}if(from){where.push("COALESCE(m.local_release_date,m.release_date)>=?");p.push(from);}if(to){where.push("COALESCE(m.local_release_date,m.release_date)<=?");p.push(to);}
