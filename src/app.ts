...
export async function handleRequest(request:Request,env:Env,ctx:ExecutionContextLike):Promise<Response>{
  ...
  if(path==="/login")return redirect("/calendar");
  if(path==="/logout")return redirect("/cdn-cgi/access/logout");

  if(path==="/admin"&&request.method==="GET")return redirect("/calendar");
  ...
  if(path==="/admin/sync"&&request.method==="POST")return requireAdmin(request,env,async()=>{const f=await request.formData();ctx.waitUntil(runManualSync(env,String(f.get("kind")||"shows")));return redirect(safeReturn(request,"/calendar"));});

  const groupAction=path.match(/^\/admin\/series-groups\/([a-z0-9-]+)\/(follow|unfollow)$/);
  if(groupAction&&request.method==="POST")return requireAdmin(request,env,async()=>{
    const slug=groupAction[1],action=groupAction[2],group=getSeriesGroup(slug);
    if(!group)return htmlResponse(layout("Collection not found",empty("Unknown series collection."),request,env),404);
    if(action==="follow"){
      try{await syncShowSearch(env,group.rootName);}catch(e){console.warn("TVmaze collection refresh failed",e);}
      await subscribeSeriesGroup(env,slug);
    }else await unsubscribeSeriesGroup(env,slug);
    return redirect(safeReturn(request,"/series"));
  });
  ...
  if(seriesAction&&request.method==="POST")return requireAdmin(request,env,async()=>{
    ...
    if(seriesAction[2]==="follow")await followShowManually(env,id);
    else await unfollowShowManually(env,id);
    return redirect(safeReturn(request,"/series"));
  });
  ...
  if(movieAction&&request.method==="POST")return requireAdmin(request,env,async()=>{
    ...
    return redirect(safeReturn(request,"/movies"));
  });
...
function requireAdmin(request:Request,env:Env,fn:()=>Response|Promise<Response>):Response|Promise<Response>{
  if(!isAdmin(request,env))return htmlResponse(layout("Access required",`<section class="panel narrow"><p class="eyebrow">Private area</p><h1>Cloudflare Access required</h1><p class="lede">Release Radar is protected by Cloudflare Access.</p></section>`,request,env),403);
  return fn();
}
...
function safeReturn(request:Request,fallback:string):string{
  ...
  try{const u=new URL(ref);return u.origin===new URL(request.url).origin?`${u.pathname}${u.search}`:fallback;}catch{return fallback;}
}
...
async function seriesBrowse(url:URL,request:Request,env:Env):Promise<Response>{
  ...
  const [rows,groups]=await Promise.all([
    all<ShowRow>(env.DB,`SELECT s.*,EXISTS(SELECT 1 FROM followed_shows f WHERE f.show_id=s.id) AS followed FROM shows s ${where.length?`WHERE ${where.join(" AND ")}`:""} ORDER BY ${orders[sort]||orders.rating} LIMIT ? OFFSET ?`,[...p,PAGE_SIZE,(page-1)*PAGE_SIZE]),
    getSeriesGroupCards(env,q,calendar==="saved")
  ]);
  ...
  return htmlResponse(layout("Series",`${pageHead("Series","Find your next obsession","Search uses live TVmaze results. Collection tiles let you follow an entire franchise, including future series.")}${form}${withSeriesGroups(showGrid(rows),groups)}${pager(url,page,rows.length===PAGE_SIZE)}`,request,env));
}
...
async function adminSeries(url:URL,request:Request,env:Env):Promise<Response>{
  ...
  const [rows,groups]=await Promise.all([
    all<ShowRow>(env.DB,`SELECT s.*,EXISTS(SELECT 1 FROM followed_shows f WHERE f.show_id=s.id) AS followed FROM shows s ${where.length?`WHERE ${where.join(" AND ")}`:""} ORDER BY COALESCE(s.weight,0) DESC,COALESCE(s.rating,0) DESC LIMIT 60`,p),
    getSeriesGroupCards(env,q,calendar==="saved")
  ]);
  ...
  return htmlResponse(layout("Series calendar",`${pageHead("Series calendar","Pick your shows","Follow one series, or subscribe to a whole franchise collection that automatically picks up future shows.")}${form}${withSeriesGroups(manageShowGrid(rows),groups)}`,request,env));
}
...
async function adminMovies(url:URL,request:Request,env:Env):Promise<Response>{
  ...
  return htmlResponse(layout("Movies calendar",`${pageHead("Movies calendar","Pick the releases you care about","Adding a movie puts its Czech release date into the Movies calendar feed.")}${form}${manageMovieGrid(rows)}`,request,env));
}
...
async function calendarPage(request:Request,env:Env,privateMode:boolean):Promise<Response>{
  ...
  const feed=(kind:string,title:string,url:string,count:number,items:number)=>`<section class="calendar-card"><div class="calendar-icon">${kind==="series"?"▣":"▶"}</div><p class="eyebrow">${esc(kind)}</p><h2>${esc(title)}</h2><p>${count} saved · ${items} upcoming calendar events.</p>${url?`<div class="feed-url"><code>${esc(url)}</code><button type="button" data-copy="${attr(url)}">Copy feed URL</button></div><div class="actions"><a class="button secondary" href="/admin/${kind}">Choose ${kind}</a></div>`:`<div class="lock-card"><strong>Private link hidden</strong><p>Open Calendar through Cloudflare Access to reveal and copy this feed URL.</p></div>`}</section>`;
  const body=`<section class="calendar-hero"><p class="eyebrow">My Calendar</p><h1>Add once. Never hunt for release dates again.</h1><p class="lede">Release Radar publishes two live calendar feeds. Add them to Google Calendar, Apple Calendar or Outlook once. When you add or remove a series, franchise collection or movie here, those calendars update automatically.</p></section><div class="calendar-cards">${feed("series","Release Radar - Series",seriesUrl,seriesCount,episodeCount)}${feed("movies","Release Radar - Movies",movieUrl,movieCount,movieEventCount)}</div><section class="panel"><p class="eyebrow">Choose content</p><h2>What goes into each calendar?</h2><div class="actions"><a class="button primary" href="/admin/series">Choose series & collections</a><a class="button primary" href="/admin/movies">Choose movies</a></div></section><div class="divider"></div><section><p class="eyebrow">How to add</p><h2>Google Calendar: choose “From URL”, not “Subscribe to calendar”</h2><p class="lede">Google's “Subscribe to calendar” field expects another Google user's email address. For Release Radar, use “From URL”. Importing an .ics file is only a one-time snapshot; adding the feed by URL stays connected.</p><div class="provider-grid"><article class="provider"><h3>Google Calendar</h3><ol class="steps"><li>Open Google Calendar on desktop.</li><li>Next to <strong>Other calendars</strong>, click <strong>+</strong>.</li><li>Choose <strong>From URL</strong>.</li><li>Paste the HTTPS feed URL and choose <strong>Add calendar</strong>.</li><li>Repeat for Series and Movies.</li></ol></article><article class="provider"><h3>Apple Calendar</h3><ol class="steps"><li>Open Calendar on Mac.</li><li>Choose <strong>File → New Calendar Subscription</strong>.</li><li>Paste the feed URL.</li><li>Choose your refresh interval and save.</li></ol></article><article class="provider"><h3>Outlook</h3><ol class="steps"><li>Open Calendar.</li><li>Choose <strong>Add calendar</strong>.</li><li>Select <strong>Subscribe from web</strong>.</li><li>Paste the feed URL, name it and import.</li></ol></article></div></section>`;
  return htmlResponse(layout("Calendar",body,request,env));
}

function withSeriesGroups(gridHtml:string,groups:SeriesGroupCard[]):string{
  if(!groups.length)return gridHtml;
  const cards=groups.map(seriesGroupCard).join("");
  if(gridHtml.startsWith('<div class="grid">'))return gridHtml.replace('<div class="grid">',`<div class="grid">${cards}`);
  if(gridHtml.startsWith('<div class="grid manage-grid">'))return gridHtml.replace('<div class="grid manage-grid">',`<div class="grid manage-grid">${cards}`);
  return `<div class="grid">${cards}</div>${gridHtml}`;
}

function seriesGroupCard(group:SeriesGroupCard):string{
  const transforms=[
    "left:5%;top:10%;transform:rotate(-8deg)",
    "left:25%;top:5%;transform:rotate(-2deg)",
    "left:45%;top:8%;transform:rotate(5deg)",
    "left:22%;top:20%;transform:rotate(1deg)"
  ];
  const images=group.images.slice(0,4).map((src,index)=>`<img loading="lazy" src="${attr(src)}" alt="" style="position:absolute;width:50%;height:72%;object-fit:cover;border-radius:5px;box-shadow:0 8px 22px rgba(0,0,0,.7);${transforms[index]}">`).join("");
  const action=group.subscribed?"unfollow":"follow";
  return `<article class="card manage-card" style="background:linear-gradient(145deg,#2b0d10,#181818 55%)"><div class="poster-wrap" style="overflow:hidden;background:radial-gradient(circle at 70% 20%,rgba(229,9,20,.45),transparent 55%),#111">${images||`<div class="poster-placeholder">${esc(group.title)}</div>`}<span style="position:absolute;left:10px;bottom:10px;padding:5px 8px;border-radius:4px;background:rgba(0,0,0,.82);font-size:10px;font-weight:900;letter-spacing:.08em;text-transform:uppercase">Collection</span>${group.subscribed?`<span class="badge">Following all</span>`:""}</div><div class="card-body"><h3>${esc(group.title)}</h3><p class="meta">${group.memberCount} series · future shows included</p><p style="margin:8px 0 0;color:#aaa;font-size:12px;line-height:1.4">${esc(group.description)}</p></div><form data-calendar-toggle="series" method="post" action="/admin/series-groups/${attr(group.slug)}/${action}" style="padding:0 10px 10px"><button class="manage-action ${group.subscribed?"secondary":""}">${group.subscribed?"✓ Following all":"＋ Follow all"}</button></form></article>`;
}
